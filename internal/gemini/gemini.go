...
// QuizPrompt is the original prompt (kept for backward compatibility with old methods)
...
// QuizQuestionsOnlyPrompt is the prompt used to generate questions and topics only (first phase)
const QuizQuestionsOnlyPrompt = `Generate a comprehensive set of multiple-choice questions based on the content of these documents.

Follow these requirements exactly:

1. Create a descriptive title for the quiz that accurately reflects the main subject matter of the documents
2. Create questions covering ALL main topics and subtopics in the documents, ensuring no significant concept is omitted. Include the topic for each question (so that questions can be grouped by topic later.). DON'T reference the documents in the questions. The questions should be self-contained and understandable without needing to refer back to the documents.
3. Include a balanced distribution of question types:
   - Basic factual recall questions
   - Comprehension questions that require understanding concepts
   - Application/analysis questions that require:
...
IMPORTANT: Only generate the question text and topic. DO NOT generate answer options yet.
...
// QuizOptionsPrompt is the prompt used to generate options for a specific question (second phase)
const QuizOptionsPrompt = `Based on the content of the provided documents, generate exactly 4 multiple-choice options for the following question.

Question: %s

Follow these requirements exactly:

1. Generate exactly 4 options with EXACTLY ONE correct answer
...
// ============================================================================
// CONCURRENT QUIZ GENERATION (NEW IMPLEMENTATION)
// ============================================================================

// questionTask represents a task to generate options for a question
type questionTask struct {
	question models.GeminiQuestionWithoutOptions
	index    int
}

// optionsResult represents the result of generating options for a question
type optionsResult struct {
	index           int
	question        models.GeminiQuestionWithoutOptions
	options         []models.GeminiOption
	promptTokens    int32
	candidateTokens int32
	totalTokens     int32
	err             error
...
// ProcessDocumentsConcurrent processes documents using concurrent option generation
// Phase 1: Generate questions and topics
// Phase 2: Concurrently generate options for each question
// Returns quiz response, prompt tokens, candidate tokens, total tokens, error
func (c *Client) ProcessDocumentsConcurrent(ctx context.Context, files []DocumentFile) (*models.GeminiQuizResponse, int32, int32, int32, error) {
	// Add a timeout to the context
	ctx, cancel := context.WithTimeout(ctx, 30*time.Minute)
	defer cancel()

	log.Printf("INFO: Starting concurrent quiz generation for %d files", len(files))

	// Phase 1: Generate questions and topics only
	log.Printf("INFO: Phase 1 - Generating questions and topics")
	questionsResponse, phase1PromptTokens, phase1CandidateTokens, phase1TotalTokens, err := c.generateQuestionsOnly(ctx, files)
	if err != nil {
		return nil, phase1PromptTokens, phase1CandidateTokens, phase1TotalTokens, fmt.Errorf("failed to generate questions: %w", err)
	}

	if questionsResponse == nil || len(questionsResponse.Questions) == 0 {
		return nil, phase1PromptTokens, phase1CandidateTokens, phase1TotalTokens, fmt.Errorf("no questions generated in phase 1")
	}

	log.Printf("INFO: Phase 1 complete - Generated %d questions with title: %s", len(questionsResponse.Questions), questionsResponse.Title)

	// Phase 2: Concurrently generate options for each question
	log.Printf("INFO: Phase 2 - Generating options for %d questions concurrently", len(questionsResponse.Questions))
	...
	if err != nil {
		// Return aggregated tokens even on error
		totalPrompt := phase1PromptTokens + phase2PromptTokens
		totalCandidate := phase1CandidateTokens + phase2CandidateTokens
		totalTokens := phase1TotalTokens + phase2TotalTokens
		return nil, totalPrompt, totalCandidate, totalTokens, fmt.Errorf("failed to generate options: %w", err)
	}

	log.Printf("INFO: Phase 2 complete - Generated options for %d questions", len(completeQuestions))

	// Aggregate all token counts
	totalPromptTokens := phase1PromptTokens + phase2PromptTokens
	totalCandidateTokens := phase1CandidateTokens + phase2CandidateTokens
	totalTokens := phase1TotalTokens + phase2TotalTokens

	log.Printf("INFO: Concurrent quiz generation complete - Total tokens: %d (Prompt: %d, Candidate: %d)",
		totalTokens, totalPromptTokens, totalCandidateTokens)

	// Build final response
	finalResponse := &models.GeminiQuizResponse{
		Title:     questionsResponse.Title,
		Questions: completeQuestions,
	}

	return finalResponse, totalPromptTokens, totalCandidateTokens, totalTokens, nil
}

// generateQuestionsOnly generates questions and topics (Phase 1)
// Returns questions response, prompt tokens, candidate tokens, total tokens, error
func (c *Client) generateQuestionsOnly(ctx context.Context, files []DocumentFile) (*models.GeminiQuestionsOnlyResponse, int32, int32, int32, error) {
	// Prepare parts for Gemini
	parts := []genai.Part{genai.Text(QuizQuestionsOnlyPrompt)}
	...
	resp, err := c.model.GenerateContent(ctx, parts...)
	if err != nil {
		return nil, 0, 0, 0, fmt.Errorf("failed to generate questions: %w", err)
	}
	...
	if resp.UsageMetadata != nil {
		...
		candidateTokens = resp.UsageMetadata.CandidatesTokenCount
		totalTokens = resp.UsageMetadata.TotalTokenCount
		log.Printf("INFO: Phase 1 Token Usage: Prompt=%d, Candidates=%d, Total=%d", promptTokens, candidateTokens, totalTokens)
	}

	// Extract JSON response
	if len(resp.Candidates) == 0 || len(resp.Candidates[0].Content.Parts) == 0 {
		return nil, promptTokens, candidateTokens, totalTokens, fmt.Errorf("no content generated")
	}
	...
	if jsonText == "" {
		return nil, promptTokens, candidateTokens, totalTokens, fmt.Errorf("no JSON content found in response")
	}

	// Parse response
	var questionsResponse models.GeminiQuestionsOnlyResponse
	decoder := json.NewDecoder(strings.NewReader(jsonText))
	decoder.DisallowUnknownFields()

	if err := decoder.Decode(&questionsResponse); err != nil {
		log.Printf("ERROR: Failed to parse questions JSON: %v", err)
		log.Printf("DEBUG: Raw JSON: %s", jsonText)
		return nil, promptTokens, candidateTokens, totalTokens, fmt.Errorf("failed to parse questions JSON: %w", err)
	}

	return &questionsResponse, promptTokens, candidateTokens, totalTokens, nil
}

// processQuestionsWithWorkerPool processes questions concurrently using a worker pool
// Returns complete questions with options, prompt tokens, candidate tokens, total tokens, error
func (c *Client) processQuestionsWithWorkerPool(
	ctx context.Context,
	files []DocumentFile,
	questions []models.GeminiQuestionWithoutOptions,
	numWorkers int,
) ([]models.GeminiQuestion, int32, int32, int32, error) {
	// Create channels
	tasks := make(chan questionTask, len(questions))
	results := make(chan optionsResult, len(questions))

	// Start workers
	var wg sync.WaitGroup
	for i := 0; i < numWorkers; i++ {
		wg.Add(1)
		go c.optionsWorker(ctx, files, tasks, results, &wg, i)
	}

	// Send tasks
	for i, q := range questions {
		tasks <- questionTask{question: q, index: i}
	}
	close(tasks)
	...
	// Collect results
	resultMap := make(map[int]optionsResult)
	var aggPromptTokens, aggCandidateTokens, aggTotalTokens int32
	var errors []string
	...
	for result := range results {
		...
		if result.err != nil {
			errors = append(errors, fmt.Sprintf("question %d: %v", result.index, result.err))
			log.Printf("ERROR: Failed to generate options for question %d: %v", result.index, result.err)
			continue
		}
	...
	// Check for errors
	if len(errors) > 0 {
		return nil, aggPromptTokens, aggCandidateTokens, aggTotalTokens, fmt.Errorf("failed to generate options for some questions: %s", strings.Join(errors, "; "))
	}

	// Build final questions in order
	completeQuestions := make([]models.GeminiQuestion, 0, len(questions))
	for i := 0; i < len(questions); i++ {
		result, ok := resultMap[i]
		if !ok {
			return nil, aggPromptTokens, aggCandidateTokens, aggTotalTokens, fmt.Errorf("missing result for question %d", i)
		}

		completeQuestions = append(completeQuestions, models.GeminiQuestion{
	...
	log.Printf("INFO: Successfully generated options for %d questions using %d workers", len(completeQuestions), numWorkers)
	log.Printf("INFO: Phase 2 Token Usage: Prompt=%d, Candidates=%d, Total=%d", aggPromptTokens, aggCandidateTokens, aggTotalTokens)

	return completeQuestions, aggPromptTokens, aggCandidateTokens, aggTotalTokens, nil
}

// optionsWorker is a worker that processes question tasks
func (c *Client) optionsWorker(
	ctx context.Context,
	files []DocumentFile,
	tasks <-chan questionTask,
	results chan<- optionsResult,
	wg *sync.WaitGroup,
	workerID int,
) {
	defer wg.Done()
	...
	for task := range tasks {
		log.Printf("INFO: Worker %d processing question %d: %s", workerID, task.index, task.question.Text)

		options, pTokens, cTokens, tTokens, err := c.generateOptionsForQuestion(ctx, files, task.question.Text)
		...
		log.Printf("INFO: Worker %d completed question %d", workerID, task.index)
	}
}

// generateOptionsForQuestion generates options for a specific question
// Returns options response, prompt tokens, candidate tokens, total tokens, error
func (c *Client) generateOptionsForQuestion(
	ctx context.Context,
	files []DocumentFile,
	questionText string,
) (*models.GeminiOptionsResponse, int32, int32, int32, error) {
	// Prepare prompt with question
	promptText := fmt.Sprintf(QuizOptionsPrompt, questionText)
	parts := []genai.Part{genai.Text(promptText)}

	// Add document files (same context as phase 1)
	for _, file := range files {
		data, err := os.ReadFile(file.Path)
		if err != nil {
			return nil, 0, 0, 0, fmt.Errorf("failed to read file %s: %w", file.Name, err)
	...
	c.model.SetTopK(40)
	c.model.SetTopP(0.95)
	c.model.SetMaxOutputTokens(int32(2048))

	resp, err := c.model.GenerateContent(ctx, parts...)
	if err != nil {
		return nil, 0, 0, 0, fmt.Errorf("failed to generate options: %w", err)
	}
	...
	// Parse response
	var optionsResponse models.GeminiOptionsResponse
	decoder := json.NewDecoder(strings.NewReader(jsonText))
	decoder.DisallowUnknownFields()

	if err := decoder.Decode(&optionsResponse); err != nil {
		log.Printf("ERROR: Failed to parse options JSON: %v", err)
		log.Printf("DEBUG: Raw JSON: %s", jsonText)
		return nil, promptTokens, candidateTokens, totalTokens, fmt.Errorf("failed to parse options JSON: %w", err)
	}

	// Validate options
	if len(optionsResponse.Options) != 4 {
		return nil, promptTokens, candidateTokens, totalTokens, fmt.Errorf("expected 4 options, got %d", len(optionsResponse.Options))
	}
	...
	if correctCount != 1 {
		return nil, promptTokens, candidateTokens, totalTokens, fmt.Errorf("expected exactly 1 correct answer, got %d", correctCount)
	}

	return &optionsResponse, promptTokens, candidateTokens, totalTokens, nil
}
