...
const listPublicQuizesWithCreator = `-- name: ListPublicQuizesWithCreator :many
SELECT
    q.id,
    q.title,
    q.description,
...
type ListPublicQuizesWithCreatorRow struct {
	ID             uuid.UUID      `json:"id"`
	Title          string         `json:"title"`
	Description    pgtype.Text    `json:"description"`
	Visibility     QuizVisibility `json:"visibility"`
...
func (q *Queries) ListPublicQuizesWithCreator(ctx context.Context) ([]ListPublicQuizesWithCreatorRow, error) {
	rows, err := q.db.Query(ctx, listPublicQuizesWithCreator)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListPublicQuizesWithCreatorRow{}
	for rows.Next() {
		var i ListPublicQuizesWithCreatorRow
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.Description,
